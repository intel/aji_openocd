...
static int nios5_poll(struct target *target);
static int nios5_debug_entry(struct target *target);
static int nios5_restore_context(struct target *target, bool bpwp);
static int nios5_set_breakpoint(struct target *target,
	struct breakpoint *breakpoint, uint8_t matchmode);
static int nios5_set_context_breakpoint(struct target *target,
	struct breakpoint *breakpoint, uint8_t matchmode);
static int nios5_set_hybrid_breakpoint(struct target *target,
	struct breakpoint *breakpoint);
static int nios5_unset_breakpoint(struct target *target,
	struct breakpoint *breakpoint);
static int nios5_wait_dscr_bits(struct target *target, uint32_t mask,
	uint32_t value, uint32_t *dscr);
static int nios5_mmu(struct target *target, int *enabled);
static int nios5_mmu_modify(struct target *target, int enable);
static int nios5_virt2phys(struct target *target,
	target_addr_t virt, target_addr_t *phys);
static int nios5_read_cpu_memory(struct target *target,
	uint32_t address, uint32_t size, uint32_t count, uint8_t *buffer);


/*  restore cp15_control_reg at resume */
static int nios5_restore_cp15_control_reg(struct target *target)
{
	int retval = ERROR_OK;
	struct nios5_common *nios5 = target_to_nios5(target);
	struct armv7a_common *armv7a = target_to_armv7a(target);

	if (nios5->cp15_control_reg != nios5->cp15_control_reg_curr) {
		nios5->cp15_control_reg_curr = nios5->cp15_control_reg;
		/* LOG_INFO("cp15_control_reg: %8.8" PRIx32, nios5->cp15_control_reg); */
		retval = armv7a->arm.mcr(target, 15,
				0, 0,	/* op1, op2 */
				1, 0,	/* CRn, CRm */
				nios5->cp15_control_reg);
	}
	return retval;
}
 ...
 */
static int nios5_prep_memaccess(struct target *target, int phys_access)
{
	struct armv7a_common *armv7a = target_to_armv7a(target);
	struct nios5_common *nios5 = target_to_nios5(target);
	int mmu_enabled = 0;
	...
	if (phys_access == 0) {
		...
		nios5_mmu(target, &mmu_enabled);
		if (mmu_enabled)
			nios5_mmu_modify(target, 1);
		if (nios5->dacrfixup_mode == NIOS5_DACRFIXUP_ON) {
 ...
 */
static int nios5_post_memaccess(struct target *target, int phys_access)
{
	struct armv7a_common *armv7a = target_to_armv7a(target);
	struct nios5_common *nios5 = target_to_nios5(target);
...
static int nios5_mmu_modify(struct target *target, int enable)
{
	...
	if (enable) {
		...
		if (!(nios5->cp15_control_reg & 0x1U)) {
			LOG_ERROR("trying to enable mmu on target stopped with mmu disable");
			return ERROR_FAIL;
		}
		if ((nios5->cp15_control_reg_curr & 0x1U) == 0) {
			nios5->cp15_control_reg_curr |= 0x1U;
			need_write = 1;
		}
	} else {
		if ((nios5->cp15_control_reg_curr & 0x1U) == 0x1U) {
			nios5->cp15_control_reg_curr &= ~0x1U;
			need_write = 1;
		}
	}
 ...
 */
static int nios5_init_debug_access(struct target *target)
{
	struct armv7a_common *armv7a = target_to_armv7a(target);
	uint32_t dscr;
	int retval;
	...
	return nios5_poll(target);
}

static int nios5_wait_instrcmpl(struct target *target, uint32_t *dscr, bool force)
{
	...
	retval = nios5_wait_dscr_bits(target, DSCR_INSTR_COMP, DSCR_INSTR_COMP, dscr);
	if (retval != ERROR_OK)
 ...
 */
static int nios5_exec_opcode(struct target *target,
	uint32_t opcode, uint32_t *dscr_p)
{
	...
	retval = nios5_wait_instrcmpl(target, dscr_p, false);
	if (retval != ERROR_OK)
		return retval;
	...
	/* Wait for InstrCompl bit to be set */
	retval = nios5_wait_instrcmpl(target, &dscr, true);
	if (retval != ERROR_OK) {
		LOG_ERROR("Error waiting for nios5_exec_opcode");
		return retval;
	}
...
static int nios5_vjtag_write_memap_register_u32(struct target *target,
	uint32_t address,
	uint32_t value)
{
	int retval;
...
static inline struct nios5_common *dpm_to_a(struct arm_dpm *dpm)
{
	return container_of(dpm, struct nios5_common, armv7a_common.dpm);
}

static int nios5_write_dcc(struct nios5_common *a, uint32_t data)
{
...
static int nios5_read_dcc(struct nios5_common *a, uint32_t *data,
	uint32_t *dscr_p)
{
	...
	/* Wait for DTRRXfull */
	retval = nios5_wait_dscr_bits(a->armv7a_common.arm.target,
...
static int nios5_dpm_prepare(struct arm_dpm *dpm)
{
	struct nios5_common *a = dpm_to_a(dpm);
	uint32_t dscr;
	int retval;

	/* set up invariant:  INSTR_COMP is set after ever DPM operation */
	retval = nios5_wait_instrcmpl(dpm->arm->target, &dscr, true);
	if (retval != ERROR_OK) {
...
static int nios5_dpm_finish(struct arm_dpm *dpm)
{
	/* REVISIT what could be done here? */
	return ERROR_OK;
}

static int nios5_instr_write_data_dcc(struct arm_dpm *dpm,
	uint32_t opcode, uint32_t data)
{
	...
	retval = nios5_write_dcc(a, data);
	if (retval != ERROR_OK)
		return retval;

	return nios5_exec_opcode(
			a->armv7a_common.arm.target,
			opcode,
			&dscr);
}

static int nios5_instr_write_data_rt_dcc(struct arm_dpm *dpm,
...
static int nios5_instr_write_data_r0(struct arm_dpm *dpm,
	uint32_t opcode, uint32_t data)
{
	struct nios5_common *a = dpm_to_a(dpm);
	uint32_t dscr = DSCR_INSTR_COMP;
	...
	retval = nios5_instr_write_data_rt_dcc(dpm, 0, data);
	if (retval != ERROR_OK)
		return retval;
...
static int nios5_instr_cpsr_sync(struct arm_dpm *dpm)
{
	struct target *target = dpm->arm->target;
	uint32_t dscr = DSCR_INSTR_COMP;

	/* "Prefetch flush" after modifying execution status in CPSR */
	return nios5_exec_opcode(target,
			ARMV4_5_MCR(15, 0, 0, 7, 5, 4),
			&dscr);
}

static int nios5_instr_read_data_dcc(struct arm_dpm *dpm,
	uint32_t opcode, uint32_t *data)
{
	struct nios5_common *a = dpm_to_a(dpm);
	int retval;
	...
	return nios5_read_dcc(a, data, &dscr);
}

static int nios5_instr_read_data_rt_dcc(struct arm_dpm *dpm,
	uint8_t rt, uint32_t *data)
{
	struct nios5_common *a = dpm_to_a(dpm);
	uint32_t dscr = DSCR_INSTR_COMP;
...
static int nios5_instr_read_data_r0(struct arm_dpm *dpm,
	uint32_t opcode, uint32_t *data)
{
	struct nios5_common *a = dpm_to_a(dpm);
	uint32_t dscr = DSCR_INSTR_COMP;
	...
	/* write R0 to DCC */
	return nios5_instr_read_data_rt_dcc(dpm, 0, data);
}

static int nios5_bpwp_enable(struct arm_dpm *dpm, unsigned index_t,
	uint32_t addr, uint32_t control)
{
	...
	retval = nios5_vjtag_write_memap_register_u32(dpm->arm->target,
			vr, addr);
	if (retval != ERROR_OK)
		return retval;
	retval = nios5_vjtag_write_memap_register_u32(dpm->arm->target,
...
static int nios5_bpwp_disable(struct arm_dpm *dpm, unsigned index_t)
{
	struct nios5_common *a = dpm_to_a(dpm);
	uint32_t cr;
	...
	return nios5_vjtag_write_memap_register_u32(dpm->arm->target, cr, 0);
}

static int nios5_dpm_setup(struct nios5_common *a, uint32_t didr)
{
	...
	dpm->prepare = nios5_dpm_prepare;
	dpm->finish = nios5_dpm_finish;

	dpm->instr_write_data_dcc = nios5_instr_write_data_dcc;
	dpm->instr_write_data_r0 = nios5_instr_write_data_r0;
	dpm->instr_cpsr_sync = nios5_instr_cpsr_sync;

	dpm->instr_read_data_dcc = nios5_instr_read_data_dcc;
	dpm->instr_read_data_r0 = nios5_instr_read_data_r0;

	dpm->bpwp_enable = nios5_bpwp_enable;
	dpm->bpwp_disable = nios5_bpwp_disable;
...
static struct target *get_nios5(struct target *target, int32_t coreid)
{
...
static int nios5_halt(struct target *target);

static int nios5_halt_smp(struct target *target)
{
	int retval = 0;
	struct target_list *head;
	struct target *curr;
...
static int nios5_poll(struct target *target)
{
	...
	uint32_t dscr;
	struct nios5_common *nios5 = target_to_nios5(target);
	struct armv7a_common *armv7a = &nios5->armv7a_common;
	...
	if ((target->state == TARGET_HALTED) && (target->smp) &&
		...
		target->gdb_service->target =
			get_nios5(target, target->gdb_service->core[1]);
	...
	if (DSCR_RUN_MODE(dscr) == (DSCR_CORE_HALTED | DSCR_CORE_RESTARTED)) {
		if (prev_target_state != TARGET_HALTED) {
			...
			retval = nios5_debug_entry(target);
			if (retval != ERROR_OK)
				return retval;
...
static int nios5_halt(struct target *target)
{
	...
	retval = nios5_wait_dscr_bits(target, DSCR_CORE_HALTED,
...
static int nios5_internal_restore(struct target *target, int current,
	target_addr_t *address, int handle_breakpoints, int debug_execution)
{
	...
	retval = nios5_restore_cp15_control_reg(target);
	if (retval != ERROR_OK)
		return retval;
	retval = nios5_restore_context(target, handle_breakpoints);
	if (retval != ERROR_OK)
		return retval;
...
static int nios5_internal_restart(struct target *target)
{
	struct armv7a_common *armv7a = target_to_armv7a(target);
	struct arm *arm = &armv7a->arm;
	int retval;
	...
	dscr = 0; /* force read of dscr */
	retval = nios5_wait_dscr_bits(target, DSCR_CORE_RESTARTED,
...
static int nios5_restore_smp(struct target *target, int handle_breakpoints)
{
	int retval = 0;
	struct target_list *head;
	struct target *curr;
	...
	while (head != (struct target_list *)NULL) {
		...
		if ((curr != target) && (curr->state != TARGET_RUNNING)
			...
			retval += nios5_internal_restore(curr, 1, &address,
					handle_breakpoints, 0);
			retval += nios5_internal_restart(curr);
		}
		head = head->next;
...
static int nios5_resume(struct target *target, int current,
	target_addr_t address, int handle_breakpoints, int debug_execution)
{
	int retval = 0;
	...
	nios5_internal_restore(target, current, &address, handle_breakpoints, debug_execution);
	if (target->smp) {
		target->gdb_service->core[0] = -1;
		retval = nios5_restore_smp(target, handle_breakpoints);
		if (retval != ERROR_OK)
			return retval;
	}
	nios5_internal_restart(target);
...
static int nios5_debug_entry(struct target *target)
{
	...
	LOG_DEBUG("dscr = 0x%08" PRIx32, nios5->cpudbg_dscr);
	...
	arm_dpm_report_dscr(&armv7a->dpm, nios5->cpudbg_dscr);
...
/* TODO, Move this */
	...
	nios5_read_cp(target, &cp15_control_register, 15, 0, 1, 0, 0);
	LOG_DEBUG("cp15_control_register = 0x%08x", cp15_control_register);

	nios5_read_cp(target, &cp15_cacr, 15, 0, 1, 0, 2);
	LOG_DEBUG("cp15 Coprocessor Access Control Register = 0x%08x", cp15_cacr);

	nios5_read_cp(target, &cp15_nacr, 15, 0, 1, 1, 2);
...
static int nios5_post_debug_entry(struct target *target)
{
	struct nios5_common *nios5 = target_to_nios5(target);
	struct armv7a_common *armv7a = &nios5->armv7a_common;
	int retval;
	...
	if (retval != ERROR_OK)
		return retval;
	LOG_DEBUG("cp15_control_reg: %8.8" PRIx32, nios5->cp15_control_reg);
	nios5->cp15_control_reg_curr = nios5->cp15_control_reg;
	...
	if (armv7a->is_armv7r) {
		...
		armv7a->armv7a_mmu.mmu_enabled =
			(nios5->cp15_control_reg & 0x1U) ? 1 : 0;
	}
	armv7a->armv7a_mmu.armv7a_cache.d_u_cache_enabled =
		(nios5->cp15_control_reg & 0x4U) ? 1 : 0;
	armv7a->armv7a_mmu.armv7a_cache.i_cache_enabled =
		(nios5->cp15_control_reg & 0x1000U) ? 1 : 0;
	nios5->curr_mode = armv7a->arm.core_mode;
...
int nios5_set_dscr_bits(struct target *target, unsigned long bit_mask, unsigned long value)
{
	struct armv7a_common *armv7a = target_to_armv7a(target);
	uint32_t dscr;
...
static int nios5_step(struct target *target, int current, target_addr_t address,
	int handle_breakpoints)
{
	...
	if (handle_breakpoints) {
		...
		if (breakpoint)
			nios5_unset_breakpoint(target, breakpoint);
	}
	...
	if (nios5->isrmasking_mode == NIOS5_ISRMASK_ON) {
		retval = nios5_set_dscr_bits(target, DSCR_INT_DIS, DSCR_INT_DIS);
		if (ERROR_OK != retval)
			return retval;
	}

	/* Break on IVA mismatch */
	nios5_set_breakpoint(target, &stepbreakpoint, 0x04);

	target->debug_reason = DBG_REASON_SINGLESTEP;

	retval = nios5_resume(target, 1, address, 0, 0);
	if (retval != ERROR_OK)
		return retval;

	int64_t then = timeval_ms();
	while (target->state != TARGET_HALTED) {
		retval = nios5_poll(target);
		if (retval != ERROR_OK)
			return retval;
		if (target->state == TARGET_HALTED)
			break;
	...
	nios5_unset_breakpoint(target, &stepbreakpoint);

	/* Re-enable interrupts if they were disabled */
	if (nios5->isrmasking_mode == NIOS5_ISRMASK_ON) {
		retval = nios5_set_dscr_bits(target, DSCR_INT_DIS, 0);
		if (ERROR_OK != retval)
			return retval;
	}
	...
	if (breakpoint)
		nios5_set_breakpoint(target, breakpoint, 0);
	...
	struct breakpoint *breakpoint, uint8_t matchmode)
{
	...
	struct nios5_common *nios5 = target_to_nios5(target);
	struct armv7a_common *armv7a = &nios5->armv7a_common;
	struct nios5_brp *brp_list = nios5->brp_list;
	...
	if (breakpoint->type == BKPT_HARD) {
		while (brp_list[brp_i].used && (brp_i < nios5->brp_num))
			brp_i++;
		if (brp_i >= nios5->brp_num) {
		...
		retval = nios5_vjtag_write_memap_register_u32(target, armv7a->debug_base
	...
	struct breakpoint *breakpoint, uint8_t matchmode)
{
	...
	while ((brp_list[brp_i].used ||
		(brp_list[brp_i].type != BRP_CONTEXT)) && (brp_i < nios5->brp_num))
		brp_i++;
...
static int nios5_set_hybrid_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	int retval = ERROR_FAIL;
	...
	while ((brp_list[brp_1].used ||
		(brp_list[brp_1].type != BRP_CONTEXT)) && (brp_1 < nios5->brp_num))
		brp_1++;

	printf("brp(CTX) found num: %d\n", brp_1);
	if (brp_1 >= nios5->brp_num) {
		LOG_ERROR("ERROR Can not find free Breakpoint Register Pair");
		return ERROR_FAIL;
	}

	while ((brp_list[brp_2].used ||
		(brp_list[brp_2].type != BRP_NORMAL)) && (brp_2 < nios5->brp_num))
		brp_2++;

	printf("brp(IVA) found num: %d\n", brp_2);
	if (brp_2 >= nios5->brp_num) {
		LOG_ERROR("ERROR Can not find free Breakpoint Register Pair");
		return ERROR_FAIL;
	}
...
static int nios5_unset_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	int retval;
	struct nios5_common *nios5 = target_to_nios5(target);
	struct armv7a_common *armv7a = &nios5->armv7a_common;
	...
	if (breakpoint->type == BKPT_HARD) {
		if ((breakpoint->address != 0) && (breakpoint->asid != 0)) {
			...
			if ((brp_i < 0) || (brp_i >= nios5->brp_num)) {
			...
			if (retval != ERROR_OK)
				return retval;
			if ((brp_j < 0) || (brp_j >= nios5->brp_num)) {
				LOG_DEBUG("Invalid BRP number in breakpoint");
				return ERROR_OK;
			}
...
static int nios5_add_breakpoint(struct target *target,
	struct breakpoint *breakpoint)
{
	struct nios5_common *nios5 = target_to_nios5(target);

	if ((breakpoint->type == BKPT_HARD) && (nios5->brp_num_available < 1)) {
		LOG_INFO("no hardware breakpoint available");
		return ERROR_TARGET_RESOURCE_NOT_AVAILABLE;
	}

	if (breakpoint->type == BKPT_HARD)
		nios5->brp_num_available--;

	return nios5_set_breakpoint(target, breakpoint, 0x00);	/* Exact match */
}

static int nios5_add_context_breakpoint(struct target *target,
	struct breakpoint *breakpoint)
{
	struct nios5_common *nios5 = target_to_nios5(target);
	...
	return nios5_set_context_breakpoint(target, breakpoint, 0x02);	/* asid match */
}

static int nios5_add_hybrid_breakpoint(struct target *target,
	struct breakpoint *breakpoint)
{
	struct nios5_common *nios5 = target_to_nios5(target);
	...
	return nios5_set_hybrid_breakpoint(target, breakpoint);	/* ??? */
}


static int nios5_remove_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	struct nios5_common *nios5 = target_to_nios5(target);
...
static int nios5_assert_reset(struct target *target)
{
	struct armv7a_common *armv7a = target_to_armv7a(target);
...
static int nios5_deassert_reset(struct target *target)
{
	struct armv7a_common *armv7a = target_to_armv7a(target);
	int retval;
...
static int nios5_set_dcc_mode(struct target *target, uint32_t mode, uint32_t *dscr)
{
...
static int nios5_read_copro(struct target *target, uint32_t opcode,
	uint32_t *data, uint32_t *dscr)
{
	...
	retval = nios5_exec_opcode(target, opcode, dscr);
	if (retval != ERROR_OK)
		return retval;

	/* Move from R0 to DTRTX. */
	retval = nios5_exec_opcode(target, ARMV4_5_MCR(14, 0, 0, 0, 5, 0), dscr);
	if (retval != ERROR_OK)
		return retval;
	...
	retval = nios5_wait_dscr_bits(target, DSCR_DTRTX_FULL_LATCHED,
...
static int nios5_read_dfar_dfsr(struct target *target, uint32_t *dfar,
	uint32_t *dfsr, uint32_t *dscr)
{
	int retval;

	if (dfar) {
		retval = nios5_read_copro(target, ARMV4_5_MRC(15, 0, 0, 6, 0, 0), dfar, dscr);
		if (retval != ERROR_OK)
			return retval;
	}

	if (dfsr) {
		retval = nios5_read_copro(target, ARMV4_5_MRC(15, 0, 0, 5, 0, 0), dfsr, dscr);
		if (retval != ERROR_OK)
			return retval;
	}
...
static int nios5_write_copro(struct target *target, uint32_t opcode,
	uint32_t data, uint32_t *dscr)
{
	int retval;
	struct armv7a_common *armv7a = target_to_armv7a(target);
	...
	retval = nios5_exec_opcode(target, ARMV4_5_MRC(14, 0, 0, 0, 5, 0), dscr);
	if (retval != ERROR_OK)
		return retval;
	...
	retval = nios5_wait_dscr_bits(target, DSCR_DTRRX_FULL_LATCHED, 0, dscr);
	if (retval != ERROR_OK)
		return retval;
...
static int nios5_write_dfar_dfsr(struct target *target, uint32_t dfar,
	uint32_t dfsr, uint32_t *dscr)
{
	int retval;

	retval = nios5_write_copro(target, ARMV4_5_MCR(15, 0, 0, 6, 0, 0), dfar, dscr);
	if (retval != ERROR_OK)
		return retval;

	retval = nios5_write_copro(target, ARMV4_5_MCR(15, 0, 0, 5, 0, 0), dfsr, dscr);
	if (retval != ERROR_OK)
		return retval;
...
static int nios5_dfsr_to_error_code(uint32_t dfsr)
{
...
static int nios5_write_cpu_memory_slow(struct target *target,
	uint32_t size, uint32_t count, const uint8_t *buffer, uint32_t *dscr)
{
	 ...
	 * the address is aligned, nios5_write_cpu_memory_fast should be
	 * preferred.
	 * Preconditions:
	 * - Address is in R0.
	 * - R0 is marked dirty.
	...
	retval = nios5_set_dcc_mode(target, DSCR_EXT_DCC_NON_BLOCKING, dscr);
	if (retval != ERROR_OK)
		return retval;
	...
	while (count) {
		...
		retval = nios5_exec_opcode(target, ARMV4_5_MRC(14, 0, 1, 0, 5, 0), dscr);
		if (retval != ERROR_OK)
			return retval;
...
static int nios5_write_cpu_memory_fast(struct target *target,
	uint32_t count, const uint8_t *buffer, uint32_t *dscr)
{
	...
	retval = nios5_set_dcc_mode(target, DSCR_EXT_DCC_FAST_MODE, dscr);
	if (retval != ERROR_OK)
		return retval;
...
static int nios5_write_cpu_memory(struct target *target,
	...
	uint32_t count, const uint8_t *buffer)
{
	...
	retval = nios5_read_dfar_dfsr(target, &orig_dfar, &orig_dfsr, &dscr);
	if (retval != ERROR_OK)
		goto out;
	...
	if (size == 4 && (address % 4) == 0) {
		...
		retval = nios5_write_cpu_memory_fast(target, count, buffer, &dscr);
	} else {
		...
		retval = nios5_write_cpu_memory_slow(target, size, count, buffer, &dscr);
	}
...
out:
	...
	if (fault_dscr & DSCR_STICKY_ABORT_PRECISE) {
		if (final_retval == ERROR_OK) {
			...
			retval = nios5_read_dfar_dfsr(target, &fault_dfar, &fault_dfsr, &dscr);
			if (retval == ERROR_OK) {
				LOG_ERROR("data abort at 0x%08" PRIx32 ", dfsr = 0x%08" PRIx32, fault_dfar, fault_dfsr);
				final_retval = nios5_dfsr_to_error_code(fault_dfsr);
			} else
				final_retval = retval;
		}
		/* Fault destroyed DFAR/DFSR; restore them. */
		retval = nios5_write_dfar_dfsr(target, orig_dfar, orig_dfsr, &dscr);
		if (retval != ERROR_OK)
...
static int nios5_read_cpu_memory_slow(struct target *target,
	uint32_t size, uint32_t count, uint8_t *buffer, uint32_t *dscr)
{
	 ...
	 * non-word-sized objects. Avoid unaligned accesses as they do not work
	 * on memory address space without "Normal" attribute. If size == 4 and
	 * the address is aligned, nios5_read_cpu_memory_fast should be
	 * preferred.
	 * Preconditions:
	 * - Address is in R0.
	 * - R0 is marked dirty.
	...
	while (count) {
		...
		retval = nios5_exec_opcode(target, ARMV4_5_MCR(14, 0, 1, 0, 5, 0), dscr);
		if (retval != ERROR_OK)
			return retval;
...
static int nios5_read_cpu_memory_fast(struct target *target,
	uint32_t count, uint8_t *buffer, uint32_t *dscr)
{
	...
	retval = nios5_exec_opcode(target, ARMV4_5_LDC(0, 1, 0, 1, 14, 5, 0, 4), dscr);
	if (retval != ERROR_OK)
		return retval;
	...
	/* Wait for last issued instruction to complete. */
	retval = nios5_wait_instrcmpl(target, dscr, false);
	if (retval != ERROR_OK)
		return retval;
	...
	uint32_t count, uint8_t *buffer)
{
	...
	if (size == 4 && (address % 4) == 0) {
		/* We are doing a word-aligned transfer, so use fast mode. */
		retval = nios5_read_cpu_memory_fast(target, count, buffer, &dscr);
	} else {
		/* Use slow path. Adjust size for aligned accesses */
		switch (address % 4) {
			case 1:
		...
		retval = nios5_read_cpu_memory_slow(target, size, count, buffer, &dscr);
	}
...
static int nios5_read_phys_memory(struct target *target,
	...
	uint32_t count, uint8_t *buffer)
{
	...
	/* read memory through the CPU */
	nios5_prep_memaccess(target, 1);
	retval = nios5_read_cpu_memory(target, address, size, count, buffer);
	nios5_post_memaccess(target, 1);

	return retval;
}

static int nios5_read_memory(struct target *target, target_addr_t address,
	uint32_t size, uint32_t count, uint8_t *buffer)
{
	int retval;

	/* nios5 handles unaligned memory access */
	LOG_DEBUG("Reading memory at address " TARGET_ADDR_FMT "; size %" PRIu32 "; count %" PRIu32,
		address, size, count);

	nios5_prep_memaccess(target, 0);
	retval = nios5_read_cpu_memory(target, address, size, count, buffer);
	nios5_post_memaccess(target, 0);

	return retval;
}

static int nios5_write_phys_memory(struct target *target,
	target_addr_t address, uint32_t size,
	uint32_t count, const uint8_t *buffer)
{
	int retval;
	...
	/* write memory through the CPU */
	nios5_prep_memaccess(target, 1);
	retval = nios5_write_cpu_memory(target, address, size, count, buffer);
	nios5_post_memaccess(target, 1);

	return retval;
}

static int nios5_write_memory(struct target *target, target_addr_t address,
...
static int nios5_read_buffer(struct target *target, target_addr_t address,
				uint32_t count, uint8_t *buffer)
{
	uint32_t size;
...
static int nios5_write_buffer(struct target *target, target_addr_t address,
				 uint32_t count, const uint8_t *buffer)
{
	uint32_t size;
...
static int nios5_handle_target_request(void *priv)
{
...
static int nios5_examine_first(struct target *target)
{
	struct nios5_common *nios5 = target_to_nios5(target);
	struct armv7a_common *armv7a = &nios5->armv7a_common;
	...
	if (!target_was_examined(target)) {
		retval = nios5_dpm_setup(nios5, didr);
		if (retval != ERROR_OK)
			return retval;
	}

	/* Setup Breakpoint Register Pairs */
	nios5->brp_num = ((didr >> 24) & 0x0F) + 1;
	nios5->brp_num_context = ((didr >> 20) & 0x0F) + 1;
	nios5->brp_num_available = nios5->brp_num;
	free(nios5->brp_list);
	nios5->brp_list = calloc(nios5->brp_num, sizeof(struct nios5_brp));
/*	nios5->brb_enabled = ????; */
	for (i = 0; i < nios5->brp_num; i++) {
		nios5->brp_list[i].used = 0;
		if (i < (nios5->brp_num-nios5->brp_num_context))
			nios5->brp_list[i].type = BRP_NORMAL;
		else
			nios5->brp_list[i].type = BRP_CONTEXT;
		nios5->brp_list[i].value = 0;
		nios5->brp_list[i].control = 0;
		nios5->brp_list[i].BRPn = i;
	}

	LOG_DEBUG("Configured %i hw breakpoints", nios5->brp_num);
...
static int nios5_examine(struct target *target)
{
	int retval = ERROR_OK;

	/* Reestablish communication after target reset */
	retval = nios5_examine_first(target);

	/* Configure core debug access */
	if (retval == ERROR_OK)
		retval = nios5_init_debug_access(target);
...
static int nios5_init_target(struct command_context *cmd_ctx,
...
static int nios5_init_arch_info(struct target *target,
	struct nios5_common *nios5, struct adiv5_dap *dap)
{
	struct armv7a_common *armv7a = &nios5->armv7a_common;

	/* Setup struct nios5_common */
	nios5->common_magic = NIOS5_COMMON_MAGIC;
	armv7a->arm.dap = dap;

	/* register arch-specific functions */
	armv7a->examine_debug_reason = NULL;

	armv7a->post_debug_entry = nios5_post_debug_entry;

	armv7a->pre_restore_context = NULL;

	armv7a->armv7a_mmu.read_physical_memory = nios5_read_phys_memory;


/*	arm7_9->handle_target_request = nios5_handle_target_request; */

	/* REVISIT v7a setup should be in a v7a-specific routine */
	armv7a_init_arch_info(target, armv7a);
	target_register_timer_callback(nios5_handle_target_request, 1,
		TARGET_TIMER_TYPE_PERIODIC, target);

	return ERROR_OK;
}

static int nios5_target_create(struct target *target, Jim_Interp *interp)
{
	struct nios5_common *nios5;
	...
	nios5 = calloc(1, sizeof(struct nios5_common));
	if (nios5 == NULL) {
		LOG_ERROR("Out of memory");
		return ERROR_FAIL;
	}
	nios5->common_magic = NIOS5_COMMON_MAGIC;
	nios5->armv7a_common.is_armv7r = false;
	nios5->armv7a_common.arm.arm_vfp_version = ARM_VFP_V3;

	return nios5_init_arch_info(target, nios5, pc->dap);
}

static void nios5_deinit_target(struct target *target)
{
	struct nios5_common *nios5 = target_to_nios5(target);
	struct armv7a_common *armv7a = &nios5->armv7a_common;
...
static int nios5_mmu(struct target *target, int *enabled)
{
	...
	if (armv7a->is_armv7r)
		...
		*enabled = target_to_nios5(target)->armv7a_common.armv7a_mmu.mmu_enabled;
	...
	target_addr_t virt, target_addr_t *phys)
{
	...
	retval = nios5_mmu_modify(target, 1);
	if (retval != ERROR_OK)
		return retval;
...
COMMAND_HANDLER(nios5_handle_cache_info_command)
{
...
COMMAND_HANDLER(nios5_handle_dbginit_command)
{
	struct target *target = get_current_target(CMD_CTX);
	if (!target_was_examined(target)) {
	...
	return nios5_init_debug_access(target);
}

COMMAND_HANDLER(handle_nios5_mask_interrupts_command)
{
	struct target *target = get_current_target(CMD_CTX);
	struct nios5_common *nios5 = target_to_nios5(target);

	static const Jim_Nvp nvp_maskisr_modes[] = {
		{ .name = "off", .value = NIOS5_ISRMASK_OFF },
		{ .name = "on", .value = NIOS5_ISRMASK_ON },
		{ .name = NULL, .value = -1 },
	};
	const Jim_Nvp *n;
	...
	if (CMD_ARGC > 0) {
		...
		nios5->isrmasking_mode = n->value;
	}

	n = Jim_Nvp_value2name_simple(nvp_maskisr_modes, nios5->isrmasking_mode);
	command_print(CMD, "nios5 interrupt mask %s", n->name);

	return ERROR_OK;
}

COMMAND_HANDLER(handle_nios5_dacrfixup_command)
{
	struct target *target = get_current_target(CMD_CTX);
	struct nios5_common *nios5 = target_to_nios5(target);

	static const Jim_Nvp nvp_dacrfixup_modes[] = {
		{ .name = "off", .value = NIOS5_DACRFIXUP_OFF },
		{ .name = "on", .value = NIOS5_DACRFIXUP_ON },
		{ .name = NULL, .value = -1 },
	};
	const Jim_Nvp *n;
	...
	if (CMD_ARGC > 0) {
		...
		if (n->name == NULL)
			return ERROR_COMMAND_SYNTAX_ERROR;
		nios5->dacrfixup_mode = n->value;

	}

	n = Jim_Nvp_value2name_simple(nvp_dacrfixup_modes, nios5->dacrfixup_mode);
	command_print(CMD, "nios5 domain access control fixup %s", n->name);

	return ERROR_OK;
}

static const struct command_registration nios5_exec_command_handlers[] = {
	{
		.name = "cache_info",
		.handler = nios5_handle_cache_info_command,
		.mode = COMMAND_EXEC,
	...
	{
		.name = "dbginit",
		.handler = nios5_handle_dbginit_command,
		.mode = COMMAND_EXEC,
	...
	{
		.name = "maskisr",
		.handler = handle_nios5_mask_interrupts_command,
		.mode = COMMAND_ANY,
		.help = "mask nios5 interrupts",
		.usage = "['on'|'off']",
	},
	{
		.name = "dacrfixup",
		.handler = handle_nios5_dacrfixup_command,
		.mode = COMMAND_ANY,
...
static const struct command_registration nios5_command_handlers[] = {
	{
	...
	},
	{
		...
		.usage = "",
		.chain = nios5_exec_command_handlers,
	},
	COMMAND_REGISTRATION_DONE
};

struct target_type nios5_target = {
	.name = "nios5",
	.deprecated_name = "nios58",
	...
	.assert_reset = nios5_assert_reset,
	.deassert_reset = nios5_deassert_reset,

	/* REVISIT allow exporting VFP3 registers ... */
	.get_gdb_arch = arm_get_gdb_arch,
	.get_gdb_reg_list = arm_get_gdb_reg_list,

	.read_memory = nios5_read_memory,
	.write_memory = nios5_write_memory,

	.read_buffer = nios5_read_buffer,
	.write_buffer = nios5_write_buffer,
	...
	.add_breakpoint = nios5_add_breakpoint,
	.add_context_breakpoint = nios5_add_context_breakpoint,
	.add_hybrid_breakpoint = nios5_add_hybrid_breakpoint,
	.remove_breakpoint = nios5_remove_breakpoint,
	.add_watchpoint = NULL,
	.remove_watchpoint = NULL,

	.commands = nios5_command_handlers,
	.target_create = nios5_target_create,
	.target_jim_configure = adiv5_jim_configure,
	.init_target = nios5_init_target,
	.examine = nios5_examine,
	.deinit_target = nios5_deinit_target,

	.read_phys_memory = nios5_read_phys_memory,
	.write_phys_memory = nios5_write_phys_memory,
	.mmu = nios5_mmu,
	.virt2phys = nios5_virt2phys,
};
