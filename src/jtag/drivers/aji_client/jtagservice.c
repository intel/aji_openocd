...
struct jtagservice_record {
	...
	AJI_OPEN_ID* in_use_device_open_id;
						...
						const DWORD node_index) 
{
	...
	if(UINT32_MAX == tap_index) {
		...
		jtagservice.in_use_device_open_id = 0;
		...
		jtagservice.in_use_device_open_id = jtagservice.device_open_id_list[tap_index];
...
AJI_ERROR jtagservice_unlock()
{
	...
	AJI_ERROR status = AJI_NO_ERROR;
	status = c_aji_unlock(jtagservice.in_use_device_open_id);
	if(AJI_NO_ERROR != status) {
	...
	status = jtagservice_update_active_tap_record(0, (unsigned long) UINT32_MAX, false, 0);
	return AJI_NO_ERROR;
}
	...
	const DWORD tap_index ) 
{
	...
	if(tap_index == jtagservice.in_use_device_tap_position) {
...
int jtagservice_query_main(void) {
	...
	if (AJI_NO_ERROR == status) {
		...
		for(unsigned int i=0; i<hardware_count; ++i) {
			...
			LOG_INFO("        Number of devices on chain is %lu", (unsigned long) device_count);
			for(DWORD tap_position=0; tap_position<device_count; ++tap_position) {
				...
				if(AJI_NO_ERROR !=  status ) { 
				    LOG_ERROR("       Problem with getting nodes for this tap position. Returned %d (%s)", status, c_aji_error_decode(status));
				    free(hier_ids);
				    free(hub_infos);
				    continue;
				}
			   
				LOG_INFO("            Number of SLD nodes (hier_id_n)=%lu,", (unsigned long) hier_id_n); //hier_id_n = 0 and status=AJI_NO_ERROR if no SLD_HUB
				for(DWORD k=0; k<hier_id_n; ++k) { //With ARRIA10, this loop is entered for the FPGA Tap, if it has SLD nodes.
				    LOG_OUTPUT("            (B%lu) ", (unsigned long) k);
...
//#endif
				...
				if(AJI_NO_ERROR  != status) {
				    LOG_INFO("            Cannot open device. Returned %d (%s). This is not an error if the device is not SOCVHPS or FPGA Tap", status, c_aji_error_decode(status));
				    continue;
				}
				...
				*/
				...
				if(AJI_NO_ERROR  != status) {
				     LOG_ERROR("       Cannot send IDCODE instruction. Returned %d (%s)", status, c_aji_error_decode(status));
				     c_aji_unlock_lock_chain(open_id, chain_id);
				     c_aji_close_device(open_id);
				     continue;
				}
				...
				if(AJI_NO_ERROR  != status) {
				   LOG_ERROR("       Cannot receive IDCODE output. Returned %d (%s)", status, c_aji_error_decode(status));
				   c_aji_unlock_lock_chain(open_id, chain_id);
				   c_aji_close_device(open_id);
				   continue;
				}
				LOG_INFO("                 Return DR buffer reads %02X%02X%02X%02X %02X%02X%02X%02X", 
				            read_bits[3], read_bits[2], read_bits[1], read_bits[0], 
				            read_bits[7], read_bits[6], read_bits[5], read_bits[4]
				);
		   

				LOG_INFO("            (C1-3) Flush Device ...");
				c_aji_flush(open_id);
				   
				LOG_INFO("            (C1-4) Unlock Device ...");
				c_aji_unlock_lock_chain(open_id, chain_id);
				    
				LOG_INFO("            (C1-5) Close Device ...");
				c_aji_close_device(open_id);
				LOG_INFO("(C1-5) Done.");
...
//#endif
	...
	for(size_t i=0; i<num_devices; ++i) {
		...
		if (d == NULL ) {
		   fprintf(stream, " (null) []%s\n",i+1lu < num_devices? "," : "");
			...
			fprintf(stream,
				"    (%p) [ device_id=0x%08lX, mask=0x%08lX, instruction_length=%d, features=%lu, device_name=%s ]%s\n",
				d, d->device_id, d->mask, d->instruction_length, d->features, d->device_name == NULL? "(null)" : d->device_name,
				i+1lu < num_devices? "," : ""
			);
		}
	}
	...
	fprintf(stream, "device_open_id_list: (%lu)  [", (unsigned long) num_devices);
	for(size_t i=0; i<num_devices; ++i) {
		fprintf(stream,
			" <opaque pointer %p>%s\n",
