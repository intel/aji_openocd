...
static int load_usb_blaster_firmware(struct libusb_device_handle *libusb_dev,
				     struct jtagserv_lowlevel *low)
{
	struct image jtagserv_firmware_image;
	...
	jtagserv_firmware_image.base_address = 0;
	jtagserv_firmware_image.base_address_set = 0;

	int ret = image_open(&jtagserv_firmware_image, low->firmware_path, "ihex");
	if (ret != ERROR_OK) {
...
static int jtagserv_libusb_init(struct jtagserv_lowlevel *low)
{
	...
	const uint16_t vids_renum[] = { low->jtagserv_vid, 0 };
	const uint16_t pids_renum[] = { low->jtagserv_pid, 0 };
...
static struct jtagserv_lowlevel low = {
	.open = jtagserv_libusb_init,
	.close = jtagserv_libusb_quit,
	.read = jtagserv_libusb_read,
	.write = jtagserv_libusb_write,
	.flags = COPY_TDO_BUFFER,
};

struct jtagserv_lowlevel *jtagserv_register_libusb(void)
{
	return &low;
}
