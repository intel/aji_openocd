...
AJI_API AJI_ERROR c_aji_get_hardware(DWORD * hardware_count, AJI_HARDWARE * hardware_list, DWORD timeout) {
    ...
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_GET_HARDWARE);
    if (pfn == NULL) {
...
AJI_API AJI_ERROR c_aji_get_hardware2(DWORD * hardware_count, AJI_HARDWARE * hardware_list, char **server_version_info_list, DWORD timeout) {
    ...
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib,FNAME_AJI_GET_HARDWARE2);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_find_hardware(DWORD persistent_id, AJI_HARDWARE * hardware, DWORD timeout) {
    assert(c_jtag_client_lib != NULL);
    typedef AJI_ERROR (*ProdFn)(DWORD, AJI_HARDWARE*, DWORD);
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_FIND_HARDWARE);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_find_hardware_a(const char * hw_name, AJI_HARDWARE * hardware, DWORD timeout) {
    ...
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_FIND_HARDWARE_A);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_read_device_chain(AJI_CHAIN_ID chain_id, DWORD * device_count, AJI_DEVICE * device_list, _Bool auto_scan) {
    ...
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_READ_DEVICE_CHAIN);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_lock(AJI_OPEN_ID open_id, DWORD timeout, AJI_PACK_STYLE pack_style) {
    ...
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_LOCK);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_unlock_lock_chain(AJI_OPEN_ID unlock_id, AJI_CHAIN_ID lock_id) {
    ...
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_UNLOCK_LOCK_CHAIN);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_unlock(AJI_OPEN_ID open_id) {   
    ...
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_UNLOCK);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_lock_chain(AJI_CHAIN_ID chain_id, DWORD timeout) {
    ...
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_LOCK_CHAIN);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_unlock_chain(AJI_CHAIN_ID chain_id) {
    ...
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_UNLOCK_CHAIN);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_unlock_chain_lock(AJI_CHAIN_ID unlock_id, AJI_OPEN_ID lock_id, AJI_PACK_STYLE pack_style) {
    ...
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_UNLOCK_CHAIN_LOCK);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_open_device(AJI_CHAIN_ID chain_id, DWORD tap_position, AJI_OPEN_ID * open_id, const AJI_CLAIM * claims, DWORD claim_n, const char * application_name){
    ...
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_OPEN_DEVICE);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_close_device(AJI_OPEN_ID open_id) {
    assert(c_jtag_client_lib != NULL);
    typedef AJI_ERROR (*ProdFn)(AJI_OPEN_ID);
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_CLOSE_DEVICE);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_open_entire_device_chain(AJI_CHAIN_ID chain_id, AJI_OPEN_ID * open_id, AJI_CHAIN_TYPE style, const char * application_name) {
    ...
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_OPEN_ENTIRE_DEVICE_CHAIN);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_test_logic_reset(AJI_OPEN_ID open_id) {
    assert(c_jtag_client_lib != NULL);
    typedef AJI_ERROR (*ProdFn)(AJI_OPEN_ID*);
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_TEST_LOGIC_RESET);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_delay(AJI_OPEN_ID open_id, DWORD timeout_microseconds) {
    ...
    ProdFn pfn = (ProdFn) GetProcAddress(c_jtag_client_lib, FNAME_AJI_DELAY);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_run_test_idle(AJI_OPEN_ID open_id, DWORD num_clocks) {
    assert(c_jtag_client_lib != NULL);
    typedef AJI_ERROR(*ProdFn)(AJI_OPEN_ID, DWORD);
    ProdFn pfn = (ProdFn)GetProcAddress(c_jtag_client_lib, FNAME_AJI_RUN_TEST_IDLE);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
inline 
AJI_API AJI_ERROR c_aji_run_test_idle_a(AJI_OPEN_ID open_id, DWORD num_clocks, DWORD flags) {
    assert(c_jtag_client_lib != NULL);
    typedef AJI_ERROR(*ProdFn)(AJI_OPEN_ID, DWORD, DWORD);
    ProdFn pfn = (ProdFn)GetProcAddress(c_jtag_client_lib, FNAME_AJI_RUN_TEST_IDLE_A);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_access_ir(AJI_OPEN_ID open_id, DWORD instruction, DWORD * captured_ir, DWORD flags) {
    ...
    ProdFn pfn = (ProdFn)GetProcAddress(c_jtag_client_lib, FNAME_AJI_ACCESS_IR);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_access_ir_a(AJI_OPEN_ID open_id, DWORD length_ir, const BYTE * write_bits, BYTE * read_bits, DWORD flags) {
    ...
    ProdFn pfn = (ProdFn)GetProcAddress(c_jtag_client_lib, FNAME_AJI_ACCESS_IR_A);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_access_ir_multiple(DWORD num_devices, const AJI_OPEN_ID * open_id, const DWORD * instructions, DWORD * captured_irs) {
    ...
    ProdFn pfn = (ProdFn)GetProcAddress(c_jtag_client_lib, FNAME_AJI_ACCESS_IR_MULTIPLE);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
...
AJI_API AJI_ERROR c_aji_access_dr(AJI_OPEN_ID open_id, DWORD length_dr, DWORD flags, DWORD write_offset, DWORD write_length, const BYTE * write_bits, DWORD read_offset, DWORD read_length, BYTE * read_bits) {
    ...
    ProdFn pfn = (ProdFn)GetProcAddress(c_jtag_client_lib, FNAME_AJI_ACCESS_DR);
    if (pfn == NULL) {
        LOG_ERROR("Cannot find function '%s'", __FUNCTION__);
        return AJI_FAILURE;
    }
