...
AJI_ERROR AJI_API c_aji_open_node             (AJI_CHAIN_ID         chain_id,
    ...
    ProdFn pfn = (ProdFn)(void*)GetProcAddress(c_jtag_client_lib, FNAME_AJI_OPEN_NODE__LINUX64);
    assert(pfn != NULL);
    if (pfn == NULL) {
        return AJI_FAILURE;
    }
...
AJI_ERROR AJI_API c_aji_open_node_a           (AJI_CHAIN_ID         chain_id,
    ...
    ProdFn pfn = (ProdFn)(void*)GetProcAddress(c_jtag_client_lib, FNAME_AJI_OPEN_NODE_A__LINUX64);
    assert(pfn != NULL);
    if (pfn == NULL) {
        return AJI_FAILURE;
    }
