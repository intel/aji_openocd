...
{   LOG_DEBUG("***> IN %s(%d): %s cmd='%s'\n", __FILE__, __LINE__, __FUNCTION__, name);
	for (struct command *cc = head; cc; cc = cc->next) {
		if (strcmp(cc->name, name) == 0) {
		    LOG_DEBUG("******> IN %s(%d): %s Found cmd='%s'\n", __FILE__, __LINE__, __FUNCTION__, name);
			return cc;
		}
	}
    LOG_DEBUG("******> IN %s(%d): %s NOT Found cmd='%s'\n", __FILE__, __LINE__, __FUNCTION__, name);
	return NULL;
}
...
{   LOG_DEBUG("***> IN %s(%d): %s\n", __FILE__, __LINE__, __FUNCTION__);
    LOG_DEBUG("***> IN %s(%d): %s cmd='%s' argc=%d\n", __FILE__, __LINE__, __FUNCTION__, argc == 0? "<none>": Jim_GetString(argv[0], NULL), argc);
	if (0 == argc)  { LOG_DEBUG("***> IN %s(%d): %s Returning because stop condition (argc==0) satisfied\n", __FILE__, __LINE__, __FUNCTION__);
		return argc;
    }
	const char *cmd_name = Jim_GetString(argv[0], NULL);
	struct command *c = command_find(head, cmd_name);
	LOG_DEBUG("*********> Return from command_find(), %p\n", c);
	if (NULL == c) {   LOG_DEBUG("******> IN %s(%d): %s cmd='%s' not found\n", __FILE__, __LINE__, __FUNCTION__, Jim_GetString(argv[0], NULL));
		return argc;
	}
	LOG_DEBUG("******> IN %s(%d): %s cmd='%s' Found, find next with  argc=%d argv[1]=%s \n", __FILE__, __LINE__, __FUNCTION__, Jim_GetString(argv[0], NULL), argc-1, argc == 1? "<none>": Jim_GetString(argv[1],NULL));
	*out = c;
	LOG_DEBUG("*********> NEXT recursion of %s", __FUNCTION__);
